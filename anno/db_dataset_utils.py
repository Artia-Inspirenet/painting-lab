...
def export_dataset(db, denormalize=False):
  ...
  if denormalize:
    ...
    for anno in annotations:
      ...
      if anno['image_id'] not in image_unique_idx:
        image_unique_idx.update({anno['image_id']:1})
        
      else:
        image_unique_idx[anno['image_id']] = image_unique_idx[anno['image_id']] + 1
        
      # anno['id'] = int(anno['image_id'] + str(image_unique_idx[anno['image_id']]))
        anno['id'] = int(anno['image_id'] + "{0:02d}".format(image_unique_idx[anno['image_id']]))
        # instance_id(= anno['id']) : image_id + # of instances(two-digit)
        # ex) 1001 + 02 = 100102
    ...
    ###################################################
      ...
      image_width, image_height = image_id_to_w_h[anno['image_id']]
      x, y, w, h = anno['bbox']
      anno['bbox'] = [round(x * image_width, 2), round(y * image_height, 2), round(w * image_width, 2), round(h * image_height, 2)] # dongsu_changed_part : round(num,2)

      anno['image_id'] = int(anno['image_id']) # dongsu_added_part

      num_keypoints = 0 # dongsu_added_part

      if 'keypoints' in anno:
        for pidx in range(0, len(anno['keypoints']), 3):
          x, y = anno['keypoints'][pidx:pidx+2]
          anno['keypoints'][pidx:pidx+2] = [int(x * image_width), int(y * image_height)] # dongsu_changed_part : int()
          if anno['keypoints'][pidx+2] != 0: # dongsu_added_part
            num_keypoints += 1 # dongsu_added_part

      anno.update({'num_keypoints':num_keypoints}) # dongsu_added_part
